...
class RecipeTest(BaseTestCase):
    ...
    def test_editing_a_recipe(self):
        ...
        self.recipe = {"name": "African Tea",
                       "description": "Add 2spoonfuls of..."}
        response = self.app.put(
            "/categories/1/recipes/1", headers=self.make_token(), data=self.recipe)
        msg = str(response.json['message'])
        print(self.recipe)
    ...
    def test_editing_other_users_recipe(self):
        ...
        self.recipe = {"name": "African Tea",
                       "description": "Add 2spoonfuls of..."}
        response = self.app.put(
            "/categories/1/recipes/1", headers=self.make_second_user_token(), data=self.recipe)
        msg = str(response.json['message'])
    ...
    def test_editing_none_existent_category_and_recipe(self):
        ...
        response = self.app.put(
            "/categories/100/recipes/100", headers=self.make_token(), data=self.recipe_data)
        self.assertEqual(response.status_code, 204)
